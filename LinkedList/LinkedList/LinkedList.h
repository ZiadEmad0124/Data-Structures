...
public:
    ...
    void pop_front(){
        if (is_linked_list_empty()){
            cout << "Can't remove first item, cause LinkedList is empty" << endl;
        }else{
            node<type>* curr_ptr = head;
            head = head->get_next();
            delete curr_ptr;
    ...
    void pop_back() {
        if (is_linked_list_empty()) {
            cout << "Can't remove last item, cause LinkedList is empty" << endl;
        } else {
            if (counter == 1) {
                pop_front();
            } else {
    ...
    void pop(type item) {
        if (is_linked_list_empty()) {
            cout << "Can't remove " << item << ", cause LinkedList is empty" << endl;
    ...
    void get_head(){
        if(is_linked_list_empty())
            cout << "Head not found, cause LinkedList is empty" << endl;
        else
    ...
    void search(type item) {
        if (is_linked_list_empty())
            cout << "Can't find " << item << ", cause LinkedList is empty" << endl;
    ...
    void replace(type curr_item, type new_item) {
        if (is_linked_list_empty()) {
            cout << "Can't replace " << curr_item << ", cause LinkedList is empty" << endl;
        } else {
    ...
    void clear(){
        if(is_linked_list_empty())
            cout << "Can't clear LinkedList, cause it is empty" << endl;
        else
    ...
    void display(){
        if(is_linked_list_empty()){
            cout << "Can't display LinkedList, cause it is empty" << endl;
        }else{
            node<type>* curr_ptr = head;
